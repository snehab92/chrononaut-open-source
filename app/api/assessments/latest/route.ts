...
const skillDisplayNames: Record<string, string> = {
  goal_directed_persistence: 'Goal Follow-Through',
  ...
  response_inhibition: 'Impulse Management',
  sustained_attention: 'Focus',
  working_memory: 'Mental Tracking',
  time_management: 'Time Awareness',
  emotional_control: 'Emotional Steadiness',
};
  ...
  try {
    ...
    if (efResult.data) {
      ...
      executive_function = {
        ...
        max_score: 84,
        percentage: Math.round((ef.total_score / 84) * 100),
        skills,
    ...
    if (scResult.data) {
      ...
      self_compassion = {
        ...
        subscales,
        positive_subscales: [
          { name: 'Self-Support', score: sc.self_kindness, above_threshold: sc.self_kindness >= 3.0 },
          { name: 'Shared Experience', score: sc.common_humanity, above_threshold: sc.common_humanity >= 3.0 },
          { name: 'Balanced Awareness', score: sc.mindfulness, above_threshold: sc.mindfulness >= 3.0 },
        ],
        negative_subscales: [
          { name: 'Inner Criticism', score: sc.self_judgment, above_threshold: sc.self_judgment >= 3.0 },
          { name: 'Disconnection', score: sc.isolation, above_threshold: sc.isolation >= 3.0 },
          { name: 'Emotional Spiraling', score: sc.over_identification, above_threshold: sc.over_identification >= 3.0 },
        ],
      };
    }
    ...
    if (strengthsResult.data && strengthsResult.data.length > 0) {
      ...
      const families = ['executing', 'influencing', 'relating', 'thinking'];
