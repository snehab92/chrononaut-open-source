...
// ============================================================================
// Professional Skills - 16 Skills × Proficiency & Energy
// ============================================================================

export interface ProfessionalSkill {
  name: string;
  category: 'leading' | 'relating' | 'executing' | 'creating';
  description: string;
}

export const PROFESSIONAL_SKILLS: ProfessionalSkill[] = [
  // Leading
  { name: 'Strategic Thinking', category: 'leading', description: 'Seeing the big picture, connecting patterns, and planning for the long term' },
  { name: 'Decision Making', category: 'leading', description: 'Making timely, well-reasoned choices even with incomplete information' },
  { name: 'Influencing', category: 'leading', description: 'Persuading and inspiring others to take action or shift perspective' },
  { name: 'Visioning', category: 'leading', description: 'Creating a compelling picture of the future and rallying people toward it' },
  // Relating
  { name: 'Communication', category: 'relating', description: 'Expressing ideas clearly and listening actively to understand others' },
  { name: 'Collaboration', category: 'relating', description: 'Working effectively with others to achieve shared goals' },
  { name: 'Empathy', category: 'relating', description: 'Understanding and sharing the feelings and perspectives of others' },
  { name: 'Mentoring', category: 'relating', description: 'Guiding and developing others through sharing knowledge and experience' },
  // Executing
  { name: 'Planning & Organizing', category: 'executing', description: 'Structuring work, setting priorities, and managing time effectively' },
  { name: 'Detail Orientation', category: 'executing', description: 'Maintaining accuracy and thoroughness in your work' },
  { name: 'Resilience', category: 'executing', description: 'Recovering quickly from setbacks and maintaining performance under pressure' },
  { name: 'Drive', category: 'executing', description: 'Pursuing goals with energy, persistence, and determination' },
  // Creating
  { name: 'Innovation', category: 'creating', description: 'Generating novel ideas and finding new ways to approach challenges' },
  { name: 'Problem Solving', category: 'creating', description: 'Analyzing issues, identifying root causes, and crafting effective solutions' },
  { name: 'Adaptability', category: 'creating', description: 'Adjusting quickly to new situations, roles, or changing requirements' },
  { name: 'Continuous Learning', category: 'creating', description: 'Actively seeking new knowledge and skills to grow professionally' },
];

export const PROFESSIONAL_SKILLS_QUESTIONS: AssessmentQuestions = {
  type: 'strengths',
  title: 'Professional Skills Assessment',
  instructions: [
    'Rate 16 professional skills on two dimensions: Proficiency and Energy.',
    'Proficiency: How skilled you are at this (1 = Novice, 5 = Expert)',
    'Energy: How much this energizes you (1 = Draining, 5 = Energizing)',
    'Your ratings place each skill into one of four quadrants.',
  ],
  scale: {
    min: 1,
    max: 5,
  ...
  questions: PROFESSIONAL_SKILLS.map((skill, i) => ({
    id: i,
    text: skill.description,
    skillName: skill.name,
    subscale: skill.category.charAt(0).toUpperCase() + skill.category.slice(1),
  })),
};
...
export function getQuestionsForType(type: string): AssessmentQuestions | null {
  switch (type) {
    ...
    case 'strengths':
      return PROFESSIONAL_SKILLS_QUESTIONS;
    default:
      return null;
  }
}
