            ...
            results.assessmentHighlights = [
              {
                ...
                summary: `Top skills: ${latestAssessment
    ...
    if (sources.includes("recovery_score") || sources.includes("health_trends")) {
      promises.push(
        (async () => {
          ...
          if (data && data.length > 0) {
            const metrics = data.map((d: Record<string, unknown>) => ({
              metric_date: d.metric_date,
              recovery_score: d.recovery_score as number | null,
            ...
            if (metrics.length > 0) {
              ...
              // Calculate trend
              if (metrics.length >= 3) {
                const recent = metrics.slice(0, 3).map((d) => d.recovery_score || 0);
                const older = metrics.slice(-3).map((d) => d.recovery_score || 0);
