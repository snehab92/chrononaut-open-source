...
import { getAnthropicApiKey } from "@/lib/ai/anthropic";
...
/**
 * Runs the values alignment score calculation for all users.
 * Analyzes behavioral patterns against stored core values.
 */
export async function runValuesAlignment(): Promise<{
  processed: number;
  success: number;
  failed: number;
}> {
  ...
  const anthropic = new Anthropic({ apiKey: getAnthropicApiKey() || undefined });

  for (const userValues of valuesResults) {
    try {
      const score = await computeAlignmentForUser(supabase, anthropic, userValues);
