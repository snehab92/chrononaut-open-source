...
export async function syncWhoopData(
  ...
  try {
    ...
    // Whoop v2 API requires ISO 8601 datetime format
    ...
    const [recoveryResult, sleepResult, cycleResult, workoutResult] = await Promise.allSettled([
      client.getRecovery({ startDate, endDate, limit: 25 }),
      client.getSleep({ startDate, endDate, limit: 25 }),
      client.getCycles({ startDate, endDate, limit: 25 }),
      client.getWorkouts({ startDate, endDate, limit: 25 }),
    ]);
