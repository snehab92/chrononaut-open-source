...
export function ChatDrawer({ 
  ...
  // Track whether we're mid-switch to avoid caching stale data
  const isSwitchingAgent = useRef(false);

  // Save current chat to cache when messages change (skip during agent switch)
  useEffect(() => {
    if (isSwitchingAgent.current) return;
  ...
  const handleAgentChange = (newAgent: AgentType) => {
    ...
    // Prevent the save-to-cache effect from running during the switch
    isSwitchingAgent.current = true;
    ...
    // Re-enable cache saving after React processes the state updates
    requestAnimationFrame(() => {
      isSwitchingAgent.current = false;
    });
  };
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      // If the stream was empty, show an error instead of empty bubble
      if (!fullContent.trim()) {
        setMessages(prev =>
